...
module.exports.read = function(resource, options = {}) {
  ...
  if (options.auth) {
      ...
      .then(res => {
        return res.text().then(result => {
          if (isFolder(result, resource, store)) {
            return parseResult(resource, store), res;
          } else {
            return result, res;
          }
        });
    ...
    return fetcher
      ...
      .then(result => {
        if (isFolder(null, resource, store)) {
          return parseResult(resource, store), result;
        } else {
          return result.responseText, result;
        }
      });
  }
};

const isFolder = (result, resource, client) => {
  let store = client.graph;
  try {
    if (result) {
      store = rdf.graph();
